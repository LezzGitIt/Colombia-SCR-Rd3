...
# Point counts (excluding CIPAV, surveyed once) with >= 3 repeat surveys inside one closed sampling period -- also cited in the Research methods section
...
Print_tbl <- function(df, join_df = NULL, remove_field = NULL, drop_datatype = FALSE){
  ...
  if(drop_datatype && knitr::is_latex_output()){
    ...
    break_us  <- function(x) gsub("(\\\\_)", "\\1\\\\allowbreak{}", x)         # ranges: only after "_" (keep numbers intact)
    df <- df %>%
      mutate(across(everything(), tex_esc)) %>%
      mutate(across(any_of(c("Field_name", "Source")), break_all)) %>%
      mutate(across(any_of(c("Low_range", "High_range")), break_us))
    escape_latex <- FALSE
  }
  ...
  out <- df %>%
    ...
    column_spec(2, width = if (drop_datatype) "5.7cm" else "5cm")
  if(drop_datatype){
    # Functional_traits table: hand-tuned widths so Definition gets the room. Field
    # name, Source, and the two range columns hold short values, so keep them tight.
    # The last two columns are always Low/High range; Source (when present) sits
    # between Definition and them.
    n <- ncol(df)
    out <- out %>% column_spec(1, width = "3.2cm")
    if("Source" %in% names(df)) out <- out %>% column_spec(3, width = "1.6cm")
    out <- out %>% column_spec(n - 1, width = "1.7cm") %>% column_spec(n, width = "1.7cm")
  }
  out
}
```
...
#| tbl-cap: "Metadata associated with the 'Functional_traits.csv' dataset. The 'Source' column gives the origin of each trait: AVONET [@tobiasAVONETMorphologicalEcological2022], BIRDBASE [@sekerciogluBIRDBASEGlobalDataset2025], BirdLife (generation length from @birdGenerationLengthsWorlds2020; IUCN status from the HBW/BirdLife checklist), and Ayerbe-Quiñones (2018) [@ayerbe2018guia] for elevational range. See the 'Metadata' tab of the 'AVONET3_BirdTree.xlsx' for detailed definitions of the AVONET fields. The definition for 'Species_ayerbe' is not provided."
...
No data were altered or omitted to address outliers in `Bird_pcs_all.csv`, but there are some observations with high abundances (e.g., 4 records with \>50 individuals) that could bias analyses if not addressed. In `Bird_pcs_analysis.csv` these 4 records are removed (see `06_Analysis_wrangling.R` script for more details).
...
Code to reproduce figures plus the manuscript (rendered in quarto) can be found at \[Zenodo / Dryad repository\], with version history on github \[link\]
