...
Print_tbl <- function(df, join_df = NULL, remove_field = NULL, drop_datatype = FALSE){
  ...
  out <- df %>%
    ...
    column_spec(2, width = if (drop_datatype) "4cm" else "5cm")
  if(drop_datatype){
    # the last two columns are always Low/High range; a Source column (when the sheet has one) sits between Definition and them
    n <- ncol(df)
    if("Source" %in% names(df)) out <- out %>% column_spec(3, width = "1.9cm")
    out <- out %>% column_spec(n - 1, width = "2.5cm") %>% column_spec(n, width = "2.5cm")
  }
  out
}
```
